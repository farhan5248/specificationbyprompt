...
**Verdict: common cause as a pair — but a structural level as a scenario.** No behavioural divergence, no breach (76 s against 88.4 s un-excluded, 58 s against 97 s on the chart's values), so nothing about *this pair* is assignable. The follow-up re-run (`497dbe14`, 13:15) changed the reading: an independent pair on the same scenario ranged **100 s** with the same profile — same winner side, same CELL 3, no diff. A value that reproduces across independent runs is a **level**, not noise, and the todo-phase breakdown of all four sessions locates it precisely: every reading/setup item is metronomic across all four (±3 s), and the entire range lives in two first-test-only items — *Fix Guice configuration errors* (59–95 s, 11–17 tools: creating `ListProposalsActionImpl`/`ListProposalsPopupImpl`/`CucumberTestConfig` from nothing) and the *mvn verify iterate loop* (36–121 s: `R_b` went green in one cycle, `R_a` iterated — an 85 s swing on iteration-count luck). The lever is not this test case's content but the **walk order**: `No Proposals` — the degenerate, near-zero-logic case — ran fourth and passed at red unused, when it should lead the group and absorb the wiring cost. See **# Graph Changes** entry 2.
...
**Pair 1 needs no test-case fix — but its re-run upgraded it from noise to a structural finding.** As a single pair it is common cause (identical code, inside every limit); the independent 100 s re-run plus the four-session todo breakdown trace the level to the walk order — the degenerate `No Proposals` case ran fourth instead of first, so the concept case paid the impl-wiring and converge-loop cost that an empty-popup scenario would have absorbed for a fraction of the range. The fix is **reorder** (Graph Changes entry 2), not a change to the case itself.
...
2. `model:` Issues · `type:` reorder · `where:` each ListProposals group, starting with 4.1 (subtree 2) · `input:` the walk order put `Step Object - From Previous Steps` (two proposal forms + previous-step traversal, the full concept) first and `Step Object - No Proposals` fourth, where it passed at red unused. The todo-phase breakdown of all four first-test sessions (both `7459ea62` and the `497dbe14` re-run) shows the entire 88 s / 100 s range living in two first-test-only items: *Fix Guice configuration errors* (59–95 s, 11–17 tools — creating `ListProposalsActionImpl`/`ListProposalsPopupImpl`/`CucumberTestConfig` from nothing) and the *mvn verify iterate loop* (36–121 s, iteration-count luck on a from-scratch delta) · `assert:` **the degenerate case leads** (design rule 4): `No Proposals` first so the impl-class wiring attaches to a "popup will be empty" scenario with near-zero logic, then `From Previous Steps` adds only the traversal rule, reusing the wiring. Apply the same empty-case-first order to the sibling groups (4.2 already has `No Proposals - With/Without Component`; verify their walk position) · `source:` `7459ea62` + `497dbe14`, consistent 88→100 s pair range across independent runs, no functional diff — a structural level, not noise.

No other entry: no off-pick scenario carried a warn.
...
Two scenarios were re-run hours later, and the contrast is the run's sharpest finding. **First test** (`From Previous Steps`): 88 s raw → 100 s raw, both `No functional diff`, same winner side, same CELL 3 — a reproducible level. **Last test** (`Step Parameters - From Workspace`): 130 s raw (02:24 first attempt) → **16 s raw, NET 0.5 %** (11:04 re-run) — collapsed to the series floor. The todo-phase breakdown of all four first-test sessions shows why: reading/setup items are metronomic (±3 s across all four), and the range concentrates entirely in *Fix Guice configuration errors* (first-test-only impl wiring) and the *verify iterate loop* (iteration-count luck on a from-scratch delta). The first test's width is structural — walk-order — not model jitter; hence Graph Changes entry 2.
