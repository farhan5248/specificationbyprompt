...
A **second, independent** finding sits alongside it: the run's last four scenarios — the deepest nodes of the `Step object…` subtree — **climb** in green time (visible as the right-edge rise on the scaled chart). This is *not* a pair-width or NET phenomenon; the halves agree. It is an **expected trend** — per-scenario exploration cost scales with the accumulated subtree the earlier scenarios built — that was simply **more pronounced** this run than before. The run's range UCL stayed **below 60 s**; nothing breached control.
...
# The tail-climb — an expected trend, more pronounced (separate from pair-width)

Independent of any pair-range or NET question: the run's **last four scenarios climb** in green time. These are the deepest nodes of the `Step object…` chain, and they are slowest not because their pairs diverge (they agree) but because **each pays to re-orient in a subtree the earlier scenarios grew**.
...
Exploration tokens climb **+52%** (2,867 → 4,368) and the **share** of green spent orienting rises **31% → 40%**. The scenarios also nest in intrinsic depth (object → definition → parameter-set → table), so part of the rise is legitimately more code to write — but the *exploration-share* climb shows the dominant driver is **orientation cost scaling with accumulated subtree size**: each later half greps the same wildcards (`**/*IssueDetector*`, `**/*Issue*`) and now gets more hits to read, and reads a longer feature file. This is the same root as [#565][20]/[#566][21]: green re-derives the whole growing tree instead of being handed the one class contract it needs. It is an **expected** curve — "last item in a growing list is slowest" — that Rebuild90 shows **more sharply** than prior runs because this subtree ran deep. It is common cause with a structural, discovery-cost component; the lever (point green at the target `uml-class-<X>.md`) is [#565][20]/[#566][21] territory, not a per-scenario fix.
...
4. **The tail-climb is a separate, expected trend.** Orientation cost scales with subtree size; more pronounced here, but not a pair-width or NET signal, and not assignable to a test case.
...
3. **Flatten the tail (structural, [#565][20]/[#566][21]).** Point green at the target `uml-class-<X>.md` so orientation cost stays constant regardless of how many siblings the subtree accumulates.
...
## tail-climb (new this run)

**First explicit record.** The last four subtree scenarios climb in green time and, more tellingly, in **exploration share** (31% → 40%) and exploration tokens (+52%). Cause: orientation cost scales with the accumulated subtree earlier scenarios built — an expected trend, more pronounced here because the `Step object…` chain ran deep. Common cause with a structural discovery-cost component; the lever is [#565][20]/[#566][21] (scope green to the target class contract), not a per-scenario change. Worth tracking as its own variable in future runs to see whether the [#565][20]/[#566][21] work flattens it.
...
- **Does the [#565][20]/[#566][21] work flatten the tail-climb?** The tail rise is predicted to shrink once green is scoped to the target `uml-class-<X>.md` instead of re-deriving the growing subtree. A cross-run tail-slope tally before/after the fix would confirm the lever.
