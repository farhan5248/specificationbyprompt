...
1. **The first scenario is a structurally distinct point.** It founds the test-automation framework every later scenario reuses, so its pair-range carries one-time scaffolding cost and should be stratified out of the control limits. Rebuild86 makes this concrete: the lone over-UCL point is the first scenario, and its `_a` half's extra work is a *broad survey of the issue-type family* the rest of the batch then inherits.
2. **A fixed assignable cause is still fixed.** "Suite name capital" — [pbc-7576][10]'s `~/.m2` jar-hunt — keeps all of its search inside the project tree two batches later; [#415][11] holds. What pushed it over the limit here is position, not the regression of the old special cause.
3. **Token-scaling can over-promote a small range.** Pair 2's 21s raw range became the second-widest only after scaling. The selection sheet should be read against raw range too, or it will route review time to decode-volume noise.
4. **No silent stalls this batch.** Every per-minute token bucket in all four halves is non-zero; the [#417][8] stall did not recur, even on the concurrent ([#439][14]) green pair's first outing.
