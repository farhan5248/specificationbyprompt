---
title: Deming - Building Quality In
---
...
This inspired me to have Claude Code go through the same process, building quality in the same way. The junior developer's pairing carries straight over: Claude is the developer, a tester supplies the failing tests that drive each change, and a senior reviews the result afterward — they don't write the code for it. That review is a *separate pass after the run*, not Darmok's own refactor step (which only tidies Claude's methods enough to match the UML spec so the next test's examples can be found); judging whether the result is right is the human step, exactly as the junior's work was reviewed. The "Before AI" section described **no-test-dev** — testers generating their own test automation without a test automation developer; here Claude generates the main code, with the senior reviewing rather than writing. *Why* this lets code creation move further upstream — beyond the tester, toward the BSA and product owner — is Goldratt's lever, covered in [Goldratt - Diminishing A Limitation][9].
