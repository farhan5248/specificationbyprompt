...
- **Should the pair range be charted on tokens rather than wall-clock?** Tokens are the cleaner work proxy and immune to the decode-rate masking that lets a fast-decode day hide a work divergence in the time range. A token-pair-range chart alongside (or instead of) the time-pair-range chart may be the more honest ambiguity detector; the time range would then serve mainly as the input to `scale`.
