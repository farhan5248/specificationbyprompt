...
2. `model:` Issues · `type:` pin-cardinality · `where:` 4.1 Step Object - From Previous Steps · `input:` a previous step referencing a **component-less** object (`The Output file is present`) · `assert:` **DECIDED — propose the short form.** A qualified ref is the import; once the component is in scope a short ref is valid (inherits per the 4.3 rule) and is proposed as its short form only — re-proposing the long form would restate an in-scope import. The pinning row uses a *different* object than the import step (`The Input file is present` after `The daily batchjob Output file is present`) so dedup cannot mask the split · `source:` `b4143b31` warn, third split of [pbc-114][47] Fix 2.
3. `model:` Issues · `type:` pin-cardinality · `where:` 4.4 Step Parameters - From Workspace (correctCellListWorkspace path) · `input:` **(a)** a parameters-set description populated and asserted (`assert:` from-model text, not empty — **DECIDED: yes, add and assert the description**); **(b)** table-less non-Content parameter: **DECIDED — unreachable by authoring; no pin** (the 112/113 dead-branch rule) · `source:` `859caff9` warn.
