...
1. `model:` Issues · `type:` pin-cardinality · `where:` 4.1 Step Object - From Background Steps · `input:` two preceding steps naming the **same** step object (`The daily batchjob Output file` twice) · `assert:` the proposal appears **once** · `source:` `3354aca3`, functional diff, reviewed Pair 2.
2. `model:` Issues · `type:` pin-cardinality · `where:` 4.2 Step Object - From Workspace · `input:` a step object whose description has **two** lines · `assert:` **ASK** — `", "` join (winner), no separator (loser), or newline; the run does not determine which is intended · `source:` `1a440379`, functional diff, reviewed Pair 1.
3. `model:` Issues · `type:` pin-cardinality · `where:` 4.3 Step Definition - Component Inherited · `input:` a short object name whose predecessor used the **qualified** form · `assert:` **ASK** — which step object the lookup resolves to (manual component-only fallback vs previous-step enrichment) · `source:` `2d16b537`, functional diff, off-pick (1 s range).
4. `model:` Issues · `type:` derivation · `where:` 4.2 Step Object - From Workspace · `input:` `_a` reconstructed the workspace setup from the runtime log (`LOG_LEVEL=DEBUG`, `resetTestProject` counting) while `_b` read the sibling 4.1 case · `assert:` make the workspace-population setup explicit in the walk so both halves read it off the spec · `source:` `1a440379`, divergence walk, reviewed Pair 1.
