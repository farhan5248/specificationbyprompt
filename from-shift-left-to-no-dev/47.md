...
This was a single-run remediation audit of Rebuild47's 12 green-phase scenarios, not a two-run pair comparison, so there is no run-to-run range to report — the findings are about prompt remediation (did JaCoCo get used, fix-phase scope, tool-call grading) rather than the pair-range variables. On absolute green-phase work, `a56ac8b8` did far more than its failing runner required: 11 Edit/Write calls in the fix phase, only 1 strictly needed, the other 10 driven by running an rgr-refactor `validate_main.py` validator inside the green-phase fix step and chasing every compliance finding it surfaced. The remedy was a scope fence in fix.md (commit `569383e`) so green stops at the test-passing point.
