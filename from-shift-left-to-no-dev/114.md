...
**Fix 3 — `site/uml/uml-package.md`: add the missing `{Language}{Type}` rule.** This one is not a Test-Case; it is the run's two allowlist violations, and it is also a spec defect. `validate_main.py` fails with `Class 'SheepDogStepObjectRef' does not match any rule`, and refactor's only available response — move the class out of `src/main/java` — is both outside the allowlist and wrong, because main code constructs it.

Note what is *not* missing: `StepObjectRef` is a genuine Xtext non-terminal (`AsciiDoc.xtext` line 106) and is already listed among the `{Type}` values in `uml-overview.md`, and `{Language}` is defined there too. So the class name decomposes cleanly into two placeholders the model already knows. What never got written is the **rule combining them** — the package section grew `{Language}IssueProposal` for the proposal DTO but no rule for the value types a proposal carries. Add it alongside:

```
### {Language}{Type}
**Rule**: SOME class matches {Language}{Type} pattern
**Regex**: `^{Language}{Type}$`
 - `SheepDogStepObjectRef`
```

`SOME` rather than `ONE`, matching how `I{Type}` is phrased: `StepDefinitionRef` is the sibling `{Type}` with no wrapper class today, and the rule should admit one when it appears. The pattern collides with nothing else in the package — `Builder`, `Factory`, `Utility` and `IssueProposal` are none of them `{Type}` values.
...
The allowlist was right to block it: `TestStepIssueResolver` line 224 does `new SheepDogStepObjectRef(...)` from **main**, so the move would have broken main code. Both resumes reverted cleanly (`rm` the new file, `git checkout --` the rest) and both scenarios committed unchanged, at a cost of ~2.5 min of refactor per occurrence.

**The lever is not the allowlist** — widening it to `src/test/java/org/farhan/fake/` would have let a main-breaking move through. It is `site/uml/uml-package.md`, which is missing a `{Language}{Type}` rule (see *The Fix*). Both halves of the class name are already defined placeholders in `uml-overview.md` — `StepObjectRef` is an Xtext non-terminal listed among the `{Type}` values — so this is not an unmodelled class but an unwritten rule. Until it exists, every refactor pass over this package re-derives the same wrong fix and burns an allowlist attempt.

The deeper point: refactor could not have found the right fix. The validator told it a class did not belong in a package, and the only file it had permission to change was the class. **When the design spec is the defect, an allowlist that confines the agent to code guarantees the wrong repair** — the retry cannot converge, which is why the same move appeared on two consecutive scenarios.
