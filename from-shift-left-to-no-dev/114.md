...
|---|---|---|---|---|---|---|---|
| No component has existing (4.2) | `7bce92b7` | 5:22 | **4:41** | 41,819 ms | 60 s | **42 s** (raw) | **assignable** — functional diff: component extracted with `indexOf("/")` (A) vs `lastIndexOf('/')` (B). The scenario's stepdefs path has only one component level, so both rules pass it. |
...
Given ... stepdefs/daily batchjob/app/Input file.asciidoc file is created as follows
 ...
 Then The xtext plugin list proposals popup will be set as follows
      | Proposal Value                    | Proposal Id | Proposal Description |
      | The daily batchjob/app Input file | Input file  | Description          |
```

The asserted row above pins `lastIndexOf` — the winner's rule, and the one consistent with the existing `Proposal Id` column (`Input file`, the object name only). The `indexOf` rule would drop `app` from the value. This is the spec author's call, but the committed code and the surviving id column both point the same way.
...
Two warns, one shape: **a fixture that samples a free grammar variable at exactly one value.** Unlike the unreachable-branch warns of the last two runs, both differentiating inputs are legal documents a user would write, so both are worth pinning.
...
**None this run** — the fourth consecutive quiet batch.
