...
**Between Rebuild115 and Rebuild116 the specifications were regenerated through the a2g → g2a round-trip**, and that is what this run is about. The re-ordering and the collapse are **mechanical outputs of the pipeline**, not hand-authored edits: g2a merges value-excluded Test-Cases into merge groups, so plain `Scenario`s came back as `Scenario Outline`s with `Examples` blocks, in a different order. Six chartable scenarios became four. Both Wheeler signals moved at once:
...
**And the Test-Data merge did not help.** This scenario is one of the outlines g2a produced in the round-trip; it now carries two `Examples` blocks — `The daily batchjob` and `empty` — collapsing what used to be two separate scenarios. But both rows assert the *same single proposal* against the *same single fixture*, so both pass under either rule. **The merge doubled the case's input surface without adding one bit of discriminating power** — which is exactly what a value-excluded merge does when the excluded value was the only thing the two cases disagreed about.
...
Because the merge is a *generated* result, the tax is not a spec-authoring mistake to be argued about — it is a property of the round-trip meeting fixtures that were already under-sampled. g2a merges cases whose difference is a value it excludes; when the two cases being merged were *already* indistinguishable to their assertions (one step object, one expected proposal), the merge is lossless in the pipeline's terms and lossy in the test's. Which means **the lever is upstream, in the graphml, not in the generated feature file** — and it also means the [pbc-114][47]/[pbc-115][49] backlog is now load-bearing: every un-pinned ambiguity is a candidate for being merged away rather than fixed.
...
**The edit belongs in the graphml, not the feature file.** Rebuild116's specs are generated by a2g → g2a, so a hand-edit to the `.feature` is overwritten on the next round-trip. Both fixes below go into `pit.<model>.orig.graphml` and come back through the forward-engineer pipeline — which is what [`/rgr-review-specs`][48] does.
...
## round-trip regeneration (new variable this run)

The a2g → g2a round-trip is the run's dominant cause and it is visible on both charts. It re-ordered the scenarios and merged value-excluded Test-Cases into outlines, taking the batch from six cases to four. **Test-Data rows add case size unconditionally but add discriminating power only when the assertion varies with the data.** In the merged `Has component has existing`, both Examples rows assert the identical single proposal against the identical single fixture, so the outline doubled the input surface and separated nothing.

The variable to watch is not "did someone restructure the specs" but **what the round-trip does to an already-ambiguous case**: it merges away the distinction rather than exposing it, because the value it excludes is precisely the value the assertion never varied on. That makes the un-pinned ambiguities from [pbc-114][47] and [pbc-115][49] more urgent, not less — each one is a candidate to be silently merged on the next pass. Worth tracking across the next runs: does the range settle once the graphml carries discriminating assertions, or is the larger case size itself the new level?
...
- **Is the 3× range jump the merge, or the batch getting to harder scenarios?** The two are confounded this run — the round-trip landed at the same time the run moved into 4.2/4.5 territory. Rebuild117 is planned with explicit test-setup data, which changes the steps *and* the ordering again, so it will not separate them either; the clean comparison needs a run whose specs are unchanged from its predecessor.
- **Does explicit test-setup data reduce the range or just relocate the cost?** Making the setup explicit should cut the discovery `_b` spent 21 Greps on in Pair 1 — the background traversal was implicit in the fixture. But it also lengthens every scenario. If the next run's green time rises again while the range falls, that is the healthy shape [pbc-113][46] described and the outline tax is paid off.
