...
1. **"No action" is sometimes the whole answer.** The single widest pair on the sheet decomposed to pure decode-throughput noise. A case study that ends in "common cause, leave it alone" is a correct and valuable result, not a failed investigation — and the skill's step-7 gate is what produced it instead of a manufactured prompt edit.
2. **The faster run producing more tokens is the decisive tell.** When the run that finishes *sooner* emits *more* output, the gap cannot be about amount of work. This single comparison (7,971 tok in 198 s vs 5,962 tok in 268 s) settles the classification before any per-proxy table is even built — a reusable shortcut for future cases.
3. **Uniform inflation across independent phases signals a server day, not a defect.** Red, green, and refactor all stretched on Rebuild79. A spec/prompt/harness defect inflates the one phase it touches; a slow model server inflates everything. The cross-phase pattern is a cheap pre-classification signal.
4. **Wall-clock ranking can invert the truth.** Normalized by output tokens, this pair's "slow" and "fast" runs swap places. The strongest data point so far for moving the sheet's ranking column to a decode-normalized / active-time basis ([pbc-7374][5]).
