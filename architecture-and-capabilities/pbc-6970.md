...
1. **Pair-range catches single-tool-choice variance, not just multi-step drift.** [pbc-4849][2]'s divergence was a 10-vs-15 bash chain; here it is one `Agent` call vs one extra pair of `Glob`s. The signal fired the same way at 51 s.
2. **The proximate cause is recoverable from JSONL in one pass.** The tool-count survey flagged the lone `Agent` call; the timeline pinned the 51.8 s blocked window between subagent dispatch and the next parent-session tool — matching the pair-range to within a second.
3. **Subagent delegation is a cost lever the prompt currently leaves unmanaged.** An `Explore` subagent is the right move for large fan-out and the wrong move for a two-glob monorepo lookup. Without prompt guidance, the worker's choice is effectively coin-flip and shows up directly as wall-clock variance.
4. **The fix is in the producer (the prompt), not the worker.** Neither run did anything wrong — both found the files and passed. Tightening the prompt's search guidance removes the assignable cause structurally.
