...
- **Wall-clock pair-range over-reports.** A 58 s green spread decomposed to a −0.7 s active-time difference. Without the JSONL decomposition, the natural (and previously written) conclusion was a prompt bug that does not exist.
- **TTFT is the dominant term in short headless green phases.** Idle was 70–79 % of total green time in all four sessions. For a 3-minute green phase, the model is *waiting to start* for roughly two of those minutes, cumulatively.
- **Active time is recoverable from the JSONL in one pass** by classifying inter-event gaps (idle / tool / generation). Tool time (~24 s `mvn`) serves as a built-in sanity control.
