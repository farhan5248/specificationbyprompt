...
`active = total − idle = tool + generation`.

| Green session | Total | Idle (TTFT) | Tool | Gen | **Active** |
|---|---|---|---|---|---|
| `valid object name` R73 | 189.1 s | 116.3 s | 22.3 s | 24.2 s | **72.8 s** |
| `valid object name` R74 | 247.4 s | 176.5 s | 21.4 s | 24.6 s | **70.9 s** |
| `object doesn't exist` R73 | 178.0 s | 131.8 s | 21.6 s | 22.0 s | **46.3 s** |
| `object doesn't exist` R74 | 225.0 s | 173.9 s | 21.2 s | 27.1 s | **51.1 s** |

Read the primary-case rows: total differs by **+58 s**, but active differs by **−1.9 s**. The entire pair-range lives in idle (116 → 177 s, +60 s). Tool time is a clean control — ~21–22 s in all four sessions, dominated by the single `mvn verify`, stable run-to-run as you'd expect for deterministic work. Generation is ~22–27 s everywhere. **The only thing that moved is the server-wait.**
...
| Active time | flat (72.8 vs 70.9 s) | near-flat (46.3 vs 51.1 s) |
...
- **Wall-clock pair-range over-reports.** A 58 s green spread decomposed to a −2 s active-time difference. Without the JSONL decomposition, the natural (and previously written) conclusion was a prompt bug that does not exist.
- **TTFT is the dominant term in short headless green phases.** Idle was 62–77 % of total green time in all four sessions. For a 3-minute green phase, the model is *waiting to start* for roughly two of those minutes, cumulatively.
- **Active time is recoverable from the JSONL in one pass** by classifying inter-event gaps (idle / tool / generation). Tool time (~21 s `mvn`) serves as a built-in sanity control.
- **Baseline style drift is a real but separable artifact.** It changes committed line counts (guard style, logging convention, tabs vs spaces) without touching wall-clock — a reminder to attribute time to the event timeline, not the diff size.
...
- **Once active-time pair-range is wired in, which scenarios remain wide?** Re-ranking the `714137558` tab by active time would show whether *any* current pair carries a genuine spec/prompt cause, or whether the whole chart is presently measuring server latency.
