...
1. **A wide pair-range is a signal to investigate, not proof of a special cause.** This is the cleanest example in the series of the pair-range firing on **common-cause variation**. The spec was correct, the prompt was the same one that produced the fast run, and the test fixture encoded the deciding fact. The 1.8 min spread is inherent stochastic-worker noise, not a defect. Treating the signal as a fix-guarantee would have manufactured a phantom cause; treating it as an investigation-trigger correctly concluded "nothing to remove here."
2. **The proximate divergence is still recoverable from JSONL in one pass — and so is its absence.** Grep-target enumeration plus the thinking transcript identified the split (parser trace vs read-it-off-the-fixture) and the redundancy (R67 re-confirming the concatenation 5–6 times) in one cycle. The same machinery that names a special cause also confirms when there isn't one.
3. **Same-artifact, same-code, different-path is real and measurable.** 12,104 vs 7,041 output tokens producing the identical commit is the cleanest demonstration yet that green-phase cost is dominated by deliberation depth, not output volume.
4. **The right response to common cause is a system experiment, not an instance patch.** Hunting this pair (tightening the prompt, adding a directive) would over-fit to one run's behavior. The legitimate move is a distributional A/B — does `--effort` shift the whole spread? — whose success criterion is the chart narrowing, not this pair agreeing.
